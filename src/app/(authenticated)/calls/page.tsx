...
import { PlusCircle, Edit3, Trash2, Search, PhoneCall, ExternalLink, Filter } from 'lucide-react';
import { Call, sampleCalls, Opportunity, sampleOpportunities } from '@/types/crm';
...
// CallForm component
const CallForm = ({ call, opportunities, onSave, onCancel }: { call?: Call | null, opportunities: Opportunity[], onSave: (call: Call) => void, onCancel: () => void }) => {
  const [opportunityId, setOpportunityId] = useState(call?.opportunityId || '');
  const [dateTime, setDateTime] = useState(call?.dateTime ? new Date(call.dateTime).toISOString().substring(0, 16) : new Date().toISOString().substring(0, 16)); // For datetime-local input
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!opportunityId || !dateTime || !summary) {
        toast({ title: "Validation Error", description: "Opportunity, Date/Time, and Summary are required.", variant: "destructive"});
        return;
    }
    onSave({
        ...
        <Select value={opportunityId} onValueChange={setOpportunityId} required>
            ...
            <SelectContent>
                {opportunities.map(op => (
                    <SelectItem key={op.id} value={op.id}>{op.name} - {op.clientName}</SelectItem>
                ))}
            </SelectContent>
        </Select>
      </div>
              ...
              <TableBody>
                {filteredCalls.map((call) => (
                  <TableRow key={call.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>{call.participants.join(', ') || 'N/A'}</TableCell>
                    <TableCell className="text-right">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => handleEditCall(call)} className="mr-2 hover:text-primary">
                        <Edit3 className="h-4 w-4" />
                        <span className="sr-only">Edit</span>
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleDeleteCall(call.id)} className="hover:text-destructive">
